...
def run_strategy_updated(
    *,
    ...
    quadrant_defs = {
        ...
        'Growth_Falling': {
            'Growth_Falling_Rates_Bond': [0.3,'国债ETF', '十年国债ETF','30年国债ETF'],
            'Growth_Falling_Gold': [0.7,'黄金ETF'],
        },
        'Inflation_Rising': {
    ...
    if use_monetary_position_sizing:
        if '短融ETF' in all_etf_simp_df.columns and dr007_daily_return is not None:
            ...
            # 对齐DR007到ETF数据的日期范围
            dr007_aligned = dr007_daily_return.reindex(all_etf_simp_df.index).ffill()
    ...
    for dt in rebalance_dates:       
        ...
        available_assets_monthly = _to_month_end_simple_returns(window_simp_df)  # 预计算月末收益（用于宏观因子协方差估计）

        # 处理本期因子数据窗口
        factor_cols = ['CN_Growth_Change', 'CN_Inflation_Change']
        dt_prev_month_end = (pd.to_datetime(dt) - pd.offsets.MonthEnd(1))
        available_factors =  macro_factor_df.loc[:dt_prev_month_end, factor_cols]
        available_Y_X_Corrlation = available_assets_monthly.join(available_factors, how='inner').dropna()
        lookback_periods = 36  # 18个月窗口
        available_Y_X_Corrlation = available_Y_X_Corrlation.iloc[-lookback_periods:]
        beta_matrix, tvalue_matrix, resid_matrix = estimate_factor_betas_ols(available_Y_X_Corrlation[window_simp_df.columns], 
                                  available_Y_X_Corrlation[factor_cols], min_obs=24)


        # 2.对象限循环，最终计算各象限的“合成净值/收益”序列（第二层循环：中间层）
        # 临时存储各象限内部对各Class的权重分配和最终合成的 {quadrant: {Class: weight}}
        quadrant_returns_series_list = []
        quadrant_beta_series_list = []
        valid_quadrants = []
        quadrant_internal_weights = {}      
        quadrant_internal_beta = {}  
        for q_name in quadrant_defs.keys():
            ...
            for class_name in quadrant_defs[q_name]:
                ...
                class_beta_dict[class_name] = w_series @ beta_matrix.loc[live_etf_cols]
                # 用于计算收益
            ...
            class_beta_df =pd.concat(class_beta_dict,axis=1).dropna()
            # 读取预设的权重（这个内置没有设为参数）
        ...
        quadrant_beta_df = pd.concat(quadrant_beta_series_list, axis=1).dropna().T
        # 通过判断跳过的方式，这样格式不影响缩进
        ...
        cov_quadrant = estimate_covariance_matrix(q_log_ret_df,method=top_cov_estimate_ways,ewm_span=ewm_span_days)  #提供资产协方差方差
        cov_factor = estimate_covariance_matrix(available_Y_X_Corrlation[factor_cols],method='oas')   # 提供因子协方差（因子协方差仅用这种办法就好，因子数量少很多的）                
        # cov_factor = available_Y_X_Corrlation[factor_cols].cov()   # 提供因子协方差（因子协方差仅用这种办法就好，因子数量少很多的）
        w_top = None
        target_factor_budget = None
        target_quadrant_budget = None
        # 方法一：因子风险平价（基于β暴露）
        if use_factor_balance:
                # 4) 因子协方差（在因子空间）
                w_top = solve_factor_risk_parity_weights(
                    betas_df=quadrant_beta_df,
                    factor_cov_df=cov_factor,
            ...
            w_top = solve_risk_parity_weights(cov_quadrant, target_quadrant_budget)


        # 确保覆盖所有象限并重新归一化（reindex可能添加0导致sum<1）
        w_top = w_top.reindex(valid_quadrants).fillna(0.0)
        w_top = w_top / w_top.sum() if w_top.sum() > 0 else w_top

        # 对齐 beta 与权重
        betas_aligned = quadrant_beta_df.reindex(w_top.index).fillna(0.0)
        cov_f_aligned = cov_factor.reindex(index=betas_aligned.columns, columns=betas_aligned.columns).fillna(0.0)
        factor_vars = np.diag(cov_f_aligned.values)
        factor_corr = cov_f_aligned.iloc[0,1] / np.sqrt(factor_vars[0] * factor_vars[1])
        # print(factor_corr)
        # 组合因子暴露：b_p = β^T @ w
        b_p = (betas_aligned.T @ w_top)
        t_p = (betas_aligned.T @ w_top)
        print(b_p)
        # 因子协方差贡献：m = Σ_F @ b_p
        m = (cov_f_aligned @ b_p)
        # 因子风险贡献：RC_f = |b_f * m_f|
        ...
        rc_norm = (rc / rc_sum) if rc_sum > 0 else np.zeros_like(rc)
        
        # 目标预算（若未指定则等预算）
        if target_factor_budget is None:
            tgt = np.ones_like(rc_norm) / max(len(rc_norm), 1)
        else:
            tgt_series = pd.Series(target_factor_budget, index=betas_aligned.columns).reindex(betas_aligned.columns).fillna(0.0)
            tgt = tgt_series.values
            s_t = float(np.sum(tgt))
            tgt = (tgt / s_t) if s_t > 0 else (np.ones_like(rc_norm) / max(len(rc_norm), 1))
        
        # 输出日志
        factor_names = list(betas_aligned.columns)
        rc_str = ", ".join(
            [f"{factor_names[i]}:{rc_norm.iloc[i]:.1%}(目标{tgt[i]:.1%})" for i in range(len(factor_names))]
        )
        logger.info(f"[{dt.date()}] 因子风险贡献验证: {rc_str}")
    ...
    top_quadrant_for_plot = top_quadrant_w_hist.reindex(daily_w.index).ffill(limit=None)
